...
const EN_MESSAGES = {
  ...
  "manifest.description": "Browse, inspect, and resume local Codex conversations by project, with token stats, branch checks, and local-only storage.",
  ...
  "handoff.branchMismatch": "This conversation was recorded on branch `{sessionBranch}`, but the current workspace is on `{currentBranch}`. Continue in Codex anyway?",
  "handoff.branchMismatchContinue": "Continue anyway",
  ...
  "webview.allBranchesFilter": "All",
  "webview.currentBranchFilter": "Current branch",
  "webview.branchMatchesCurrent": "Recorded branch matches current branch: {branch}",
  "webview.branchDiffersCurrent": "Recorded branch {branch}; current branch is {currentBranch}",
  "webview.branchLabel": "Branch {branch}",
...
const ZH_MESSAGES: MessageDictionary = {
  "manifest.displayName": "CodexChat",
  "manifest.description": "按项目文件夹组织本地 Codex 会话，支持只读查看、Token 统计、分支检查和一键回到 Codex 继续工作。",
  ...
  "handoff.branchMismatch": "该会话记录在 `{sessionBranch}` 分支，但当前工作区位于 `{currentBranch}`。仍要在 Codex 中继续吗？",
  "handoff.branchMismatchContinue": "仍然继续",
  ...
  "webview.allBranchesFilter": "全部",
  "webview.currentBranchFilter": "当前分支",
  "webview.branchMatchesCurrent": "历史分支匹配当前分支：{branch}",
  "webview.branchDiffersCurrent": "历史分支 {branch}；当前分支是 {currentBranch}",
  "webview.branchLabel": "分支 {branch}",
